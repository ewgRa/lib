...
//        ob_start(array(&$this, "_obHandler"));
    ...
    function _correctSuperglobals()
    {
        ...
        $s = preg_replace('/%(?!5B)(?!5D)([0-9a-f]{2})/si', '%u00\\1', $_SERVER['QUERY_STRING']);
        $data = null;
        parse_str($s, $data);
        $data = $this->_ucs2EntitiesDecode($data);
        
        $_SERVER['QUERY_STRING'] = unparse_str( $data );

        if( $_SERVER['QUERY_STRING'] )
		{
			$_SERVER['REQUEST_URI'] = explode( '?', $_SERVER['REQUEST_URI'] );
			$_SERVER['REQUEST_URI'] = $_SERVER['REQUEST_URI'][0] . '?' . $_SERVER['QUERY_STRING'];
		}
	...
	function unparse_str( $Array, $in_array = false, $Prefix = '' )
	{
		$Result = array();
		$KPrefix = $Prefix;
		...
		{
			$Prefix = $KPrefix;
			if( $in_array ) $Prefix .= '[' . $k . ']';
			else $Prefix .= $k;
			if( is_array( $v ) )
			{
				$Result[] = unparse_str( $v, true, $Prefix );
			}
			else $Result[] = $Prefix . '=' . urlencode( $v );
		}
		return join( '&', $Result );
	}
?>